************************************************************************
*
*     Function ERFC             Called by: TACIT_CALC, TACIT_D, TACIT_L
*
*     Calculate the Complementary Error Function.  Routine is a modified
*     version of the DERFC code retrieved from NETLIB (www.netlib.org). 
*
...
*
      LOGICAL FIRST
      INTEGER*4 NTERF,NTERFC,NTERC2
      REAL ETA
      DOUBLE PRECISION SQEPS,SQRTPI,XMAX,TXMAX,XSML,Y,D1MACH
      DOUBLE PRECISION ERFCS(21),ERFCCS(59),ERC2CS(49)
*
*     save variables from FIRST call
*
...
*     format statement
*
 1000 FORMAT(/,' WARNING: X so big ERFC underflows, ERFC set to 0.0 ')
*
*     initialize
*
...
*
      ...
      FIRST = .FALSE.
*
*      for X .LT. XSML, ERFC(X) = 1.0 - ERF(X) 
*
      IF (X.LE.XSML) THEN
         ERFC = 2.0D0
         RETURN
...
*
     ...
     &           *(0.5D0+DCSEVL(8.D0/Y-1.D0,ERFCCS,NTERFC))
      ...
      ENDIF
*
*     for |X| .LE. 1.0, ERFC(X) = 1.0 - ERF(X) 
*
      IF (Y.LT.SQEPS) THEN
         ERFC = 1.D0 - 2.D0*X/SQRTPI
      ELSE
         ERFC = 1.D0 - X*(1.0D0 + DCSEVL(2.D0*X*X-1.D0,ERFCS,NTERF))
      ENDIF
